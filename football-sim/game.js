...
function renderBuilderRatings(pos){
  ...
  const avg = (...nums) => Math.round(nums.reduce((s,n)=>s+n,0)/nums.length);
  let ovr = 0;
  if(pos === "QB") ovr = avg(r.awareness, r.speed, r.agility, r.stamina, r.strength);
  else if(["RB","WR","CB"].includes(pos)) ovr = avg(r.speed, r.agility, r.awareness, r.stamina, r.strength);
  else ovr = avg(r.strength, r.stamina, r.awareness, r.speed, r.agility);
...
function rebuildBuilderFromInputs(){
...
function interestGate(college, recruitStars, recruitRating){
  const gate = 58 + college.stars*6;
  const starGate = college.stars - 1;
...
function bumpRecruiting(amount){
  ...
  for(const row of state.hs.interest){
    ...
    const need = row.needs[pos] ?? 0;
    const filled = row.filled[pos] ?? 0;
    if(need > 0 && filled >= need) continue;

    const offerAt = 35 + c.stars * 8;
...
function playCollegePostseason(){
  ...
  for(let i=0;i<bowls.length;i+=2){
    ...
    if(userIn){
      ...
      logLine(win ? "Bowl Result: WIN" : "Bowl Result: LOSS");
    } else {
  ...
  function simTeam(id){
    ...
    return (c.prestige) + (st.w-st.l)*6 + rnd(-8,8);
  }
...
function buildNFLScheduleForTeam(teamId, seasonNumber){
  ...
  const byeWeek = pick(weeks.slice(5,13));
  const schedule = [];
  let gi = 0;
...
function simulatePlayoffRound(seeds){
  ...
  wcWinners.sort((a,b)=>b.rec.w - a.rec.w);
...
function finishProSeason(){
  ...
  } else if(state.pro.freeAgencyEligible){
...
function requestExtension(){
  ...
  if(state.pro.contract.yearsLeft > 1){
    logLine("Extension: Usually negotiated with 1 year left. (Still generating an offer.)");
  }
  const ovr = calcOVR();
  const pos = state.player.pos;
  const market = clamp((ovr - 65) / 35, 0, 1);
...
function renderOverviewTab(){
  ...
  if(state.phase === "DRAFT"){
    html += `<div class="small muted">Draft is running. Click Advance Week to proceed (it will move you into the pros).</div>`;
  }
...
function renderRecruitTab(){
  ...
  if(state.phase === "HS"){
    ...
    const top = state.hs.interest
      .map(row=>({ row, c: COLLEGES.find(x=>x.id===row.collegeId) }))
      .filter(x=>x.c)
    ...
    if(canCommit){
      ...
      html += `<div class="small muted">Recruiting runs during HS. After Year 4, you can commit.</div>`;
    }
  ...
  if(state.phase === "PRO"){
    return `<h3>Recruiting</h3><div class="small muted">Recruiting is complete. Use Contracts for free agency and Season for schedule.</div>`;
  }
...
function renderContractTab(){
  ...
  html += `
    <div class="row">
      ...
      ${state.pro.inFreeAgency ? `<span class="pill good">IN FA</span>` : ""}
    </div>
    <div class="hr"></div>
  ...
  if(state.pro.inFreeAgency){
    ...
    if(!state.pro.offers || !state.pro.offers.length){
      html += `<div class="small">No offers generated yet.</div>`;
    } else {
      html += `<table class="table"><thead><tr><th>Team</th><th>Years</th><th>Annual</th><th>Bonus</th><th>Role</th><th></th></tr></thead><tbody>`;
      state.pro.offers.forEach((o, idx)=>{
        html += `<tr>
          <td>${getProTeam(o.teamId).name}</td>
          <td>${o.years}</td>
    ...
    html += `<div class="hr"></div><div class="small muted">Note</div><div class="small">While in Free Agency, you cannot advance weeks until you sign a deal.</div>`;
  }
...
function renderLogsTab(){
  return `
    <h3>Log</h3>
    <div class="small muted">Newest messages appear at the top.</div>
    <div class="log" id="logEl"></div>
    <div class="row">
      <button type="button" class="ghost" id="btnClearLog">Clear Log (UI only)</button>
    </div>
  `;
}
...
function wireTabInteractions(){
  ...
  document.querySelectorAll("[data-equip]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      toggleEquip(btn.getAttribute("data-equip"));
      save();
      renderCareer();
      setActiveTab("shop");
    });
  });
  document.querySelectorAll("[data-lux]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      buyLuxury(btn.getAttribute("data-lux"));
      save();
      renderCareer();
      setActiveTab("shop");
    });
  ...
  document.querySelectorAll("[data-sign]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const idx = parseInt(btn.getAttribute("data-sign"),10);
      signOffer(idx);
      save();
      renderCareer();
      setActiveTab("overview");
...
function renderTab(){
  ...
  else html = `<div class="small muted">Unknown tab.</div>`;

  out.innerHTML = html;

  if(tab === "logs"){
    state.ui.logEl = $("logEl");
  } else {
    // keep logEl if it exists
  }
...
function renderCareer(){
  ...
  const phaseName =
    state.phase === "HS" ? `HS Year ${state.year} • Week ${state.week}` :
    state.phase === "COLLEGE" ? `College Year ${state.year} • Week ${state.week}` :
    state.phase === "DRAFT" ? `Draft` :
    state.phase === "PRO" ? `Pros Season ${state.pro.seasonsPlayed + 1} • Week ${state.week}` :
    state.phase;

  $("careerSub").textContent = `${phaseName} • Record ${state.seasonWins}-${state.seasonLosses}`;
...
function wireHomeButtons(){
  ...
  $("btnContinue").addEventListener("click", ()=>{
    if(load()){
      ...
      logLine("No save found. Start a new career.");
      showScreen("home");
    }
  });
...
function wireBuilderButtons(){
  ...
  $("btnNext").addEventListener("click", ()=>{
    if(builder.step < 3){
      setBuilderStep(builder.step + 1);
      // if going into step 3, ensure builder base updated
      if(builder.step === 3){
        rebuildBuilderFromInputs();
      }
    } else {
  ...
  // changes on pos/physicals/archetype
  ["bPos","bHeight","bWeight","bAge"].forEach(id=>{
  ...
  // rating +/- buttons
  document.querySelectorAll("[data-attr][data-dir]").forEach(btn=>{
    btn.addEventListener("click", ()=>{
...
function boot(){
  ...
  // initial screen logic: if save exists, enable continue and show home
  const hasSave = !!localStorage.getItem(SAVE_KEY);
  if(!hasSave){
    $("btnContinue").classList.add("ghost");
  }

  // Start at home; if already saved, still home by default
  showScreen("home");

  // If user hits continue later, it loads.
}
